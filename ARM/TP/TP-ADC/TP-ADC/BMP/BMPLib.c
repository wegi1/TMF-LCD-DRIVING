...
typedef struct    __attribute__((__packed__))         //Nag³ówek pliku BMP
{  
...
typedef struct       __attribute__((__packed__))            //Nag³ówek zawieraj¹cy informacje o obrazie
{  
...
typedef struct   __attribute__((__packed__))                  // Tablica kolorów  
{  
...
typedef struct    __attribute__((__packed__))                   //Dane o bitmapie
{  
...
{
    const uint8_t *ImageData;  //WskaŸnik do danych binarnych o obrazie
	const BITMAPINFO *ImageHDR=(const BITMAPINFO *)Image;
	const uint8_t *rgb=(const uint8_t*)ImageHDR;
	rgb+=__builtin_offsetof(BITMAPINFO, bmiColors);
	const RGBQUAD *rgbtable=(const RGBQUAD *)rgb;
    ...
    void DecompressRGB()
	{
		...
		{
			const uint8_t *tmpimgdata=ImageData;
	...
	void  DecompressRLE4()
	{
		uint8_t __builtin_avr_swap(uint8_t nibble)
		{
			uint8_t res;
			res=((nibble & 0x0F)<<4 | (nibble & 0xF0)>>4);
			return res;
		}
