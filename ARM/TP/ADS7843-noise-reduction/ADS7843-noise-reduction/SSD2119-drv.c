...
void LCD_Rect(uint16_t x1, uint8_t y1, uint16_t x2, uint8_t y2, uint32_t color)
{
	...
	uint32_t val=(1+y2-y1)*(uint32_t)(1+x2-x1);  //Policz z ilu pikseli sk³ada siê prostok¹t
	for(;val;val--)
	{
...
static void setPixelAA(uint16_t x1, uint8_t y1, uint8_t alfaneg, uint32_t color, uint32_t bkgcolor)
{
	...
	ssd2119_SendDataByte((alfa*((LCD_RGB)color).red + alfaneg*(((LCD_RGB)bkgcolor).red)) >> 8);     //Blending kolorów
...
void LCD_SetTextAA(uint16_t x, uint8_t y, const char *tekst, const uint8_t * const font[], uint32_t color, uint32_t bkgcolor)
{
	...
	while(!(LCD_USART.INTFLAG.reg & SERCOM_SPI_INTFLAG_TXC));   //Zaczekaj na opró¿nienie bufora nadajnika SPI
	ssd2119_CS(1);
	ssd2119_SendCmdWithData(ssd2119_Entry_Mode, (ssd2119_EntryMode_Reg){.DFM=0b10, .DenMode=1, .TY=0b01, .ID=0b11, .AM=0}.word);
}
...
void LCD_CpyBitmap_RGB(uint16_t x, uint8_t y, uint16_t width, uint8_t height, const uint8_t *data)
{
	if(y >= LCD_GetMaxY()) return;   //Nie ma nic do wyœwietlenia - jesteœmy poza LCD
	if((y + height) > LCD_GetMaxY()) height=LCD_GetMaxY()-y;

	if(x >= LCD_GetMaxX()) return;   //Nie ma nic do wyœwietlenia - jesteœmy poza LCD
	uint16_t xw=width;
	if((x + width) > LCD_GetMaxX()) xw=LCD_GetMaxX()-x+2;
	...
	for(; height; height--)
	{
		for(uint16_t xpos=xw; xpos; xpos--)  //Przeœli wszystkie piksele bitmapy z wyj¹tkiem ostatniego
		{
			ssd2119_SendDataByte(*data++);  //Skopiuj dane RGB na LCD
			ssd2119_SendDataByte(*data++);
			ssd2119_SendDataByte(*data++);
		}
		data+=(width-xw)*3;  //Pomiñ dane o niewyœwietlanych pikselach bitmapy
	}
	while(!(LCD_USART.INTFLAG.reg & SERCOM_SPI_INTFLAG_TXC));  //Zaczekaj na wys³anie danych
	ssd2119_CS(1);
}
