...
#ifndef __TWI_SAMD_H__
#define __TWI_SAMD_H__
	...
	/* Public Interface - May be used in end-application: */
		/* Macros: */
			 ...
			 */
			#define TWI_BAUD_FROM_FREQ(Frequency) calculate_baud(SystemCoreClock, Frequency)
...
/* calculating the BAUD value using Fgclk,Fscl,Trise FSCL =fGCLK / (10 + BAUD +BAUDLOW + fGCLKTRISE )*/
static inline uint32_t calculate_baud(const uint32_t fgclk, const uint32_t fscl)
{
	float f_temp, f_baud;
	...
	return((uint32_t)f_baud);
		...
		/* Inline Functions: */
			 ...
			 */
			void TWI_Init(SercomI2cm* const TWI, const uint32_t Baud) ATTR_NON_NULL_PTR_ARG(1);
			 ...
			 *  \param[in] TWI  Pointer to the base of the TWI peripheral within the device.
			 */
			static inline void TWI_Disable(SercomI2cm* const TWI) ATTR_ALWAYS_INLINE ATTR_NON_NULL_PTR_ARG(1);
			static inline void TWI_Disable(SercomI2cm* const TWI)
			{
				TWI->CTRLA.bit.ENABLE=0;
				while(TWI->SYNCBUSY.bit.ENABLE != 0);
			}
			 ...
			 *  \param[in] TWI  Pointer to the base of the TWI peripheral within the device.
			 */
			static inline void TWI_StopTransmission(SercomI2cm* const TWI) ATTR_ALWAYS_INLINE ATTR_NON_NULL_PTR_ARG(1);
			static inline void TWI_StopTransmission(SercomI2cm* const TWI)
			{
		...
		/* Function Prototypes: */
			 ...
			 */
			bool TWI_SendByte(SercomI2cm* const TWI,
			 ...
			 */
			bool TWI_ReceiveByte(SercomI2cm* const TWI,
			                     uint8_t* const Byte,
			 ...
			 *  \return A value from the \ref TWI_ErrorCodes_t enum.
			 */
			uint8_t TWI_ReadPacket(SercomI2cm* const TWI,
			                       const uint8_t SlaveAddress,
			 ...
			 *  \return A value from the \ref TWI_ErrorCodes_t enum.
			 */
			uint8_t TWI_WritePacket(SercomI2cm* const TWI,
			                        const uint8_t SlaveAddress,
			                        const uint8_t TimeoutMS,
			                        const uint8_t* InternalAddress,
			                        uint8_t InternalAddressLen,
