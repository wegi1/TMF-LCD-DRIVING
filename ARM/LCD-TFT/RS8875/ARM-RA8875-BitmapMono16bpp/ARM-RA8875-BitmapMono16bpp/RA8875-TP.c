...
{
	uint16_t result=result;		//Zapobiega komunikatowi, ¿e result mo¿e byæ niezainicjowany
...
_Bool TouchPanel_GetPositionXY(TP_Position *pos)
{
	...
	pos->X=(RA_SendCmdReadData(RA_Touch_Panel_X_High_Byte_Data_Register) << 2) + (HiByte & 0b0011) ;            //Odbierz pozycjê X
	pos->Y=(RA_SendCmdReadData(RA_Touch_Panel_Y_High_Byte_Data_Register) << 2) + ((HiByte & 0x0b1100) >> 2);    //Odbierz pozycjê Y
	...
	RA_SendCmdWithData(RA_Interrupt_Control_Register2, (RS8875_INTC2_Reg){.TouchPanelFlag=true}.byte);  //Wyzeruj przerwanie, bez tego nie dzia³a auto mode
	return !tps;
}