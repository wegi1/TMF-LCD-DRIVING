...
void Text_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf(bufor, "%7lu b/s, %6lu pix/s", counter*3, counter*3/10);
...
void Line_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf(bufor, "%7lu b/s, %7lu pix/s", counter*3, counter*3/13);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			counter=0; counterrdy=false;
		}
	}
...
void Bitmap_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf(bufor, "%5lu b/s, %5lu pix/s", counter*3, counter*3/2);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			counter=0; counterrdy=false;
		}
	}
...
void Circle_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf(bufor, "%5lu b/s, %5lu pix/s", counter*3, counter*3/10);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			counter=0; counterrdy=false;
		}
	}
