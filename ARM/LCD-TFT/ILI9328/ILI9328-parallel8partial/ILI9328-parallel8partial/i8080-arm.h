...
{
	LCD_WR(0);
	i8080_D0D7.OUT.reg=b8 << 8;  //W trybie 8-bitowym wykorzystywane s¹ linie D8-D15
	LCD_WR(1);
}
...
{
	i8080_D0D7.DIRCLR.reg=0xff00;   //Prze³¹czamy port na wejœcie
	LCD_RD(0);
	delay_us(i8080_RDL);    //Dane pojawiaj¹ siê najwczeœniej po 250 ns, a niski stan CS musi trwaæ co najmniej 500 ns
	uint8_t data=(i8080_D0D7.IN.reg >> 8) & 0xff;
	LCD_RD(1);
	delay_us(i8080_RDH);
	i8080_D0D7.DIRSET.reg=0xff00;  //Z powrotem jest wyjœciem
	return data;
}
...
static __attribute__((always_inline)) inline uint16_t i8080_Read_W() //Odczytaj s³owo, zak³ada, ¿e CS jest aktywny
{
	...
	LCD_RD(0);
	delay_us(i8080_RDL);    //Dane pojawiaj¹ siê najwczeœniej po 250 ns, a niski stan CS musi trwaæ co najmniej 500 ns
	uint16_t data=i8080_D0D7.IN.reg & 0xff00;
	LCD_RD(1);
	delay_us(i8080_RDH);
	LCD_RD(0);
	delay_us(i8080_RDL);    //Dane pojawiaj¹ siê najwczeœniej po 250 ns, a niski stan CS musi trwaæ co najmniej 500 ns
	data|=((i8080_D0D7.IN.reg >> 8) & 0x00ff);
	delay_us(i8080_RDH);
	LCD_RD(1);
	i8080_D0D7.DIRSET.reg=0xff00;  //Z powrotem jest wyjœciem
	return data;
...
static inline void LCD_RESET()
{
	...
	delay_ms(1);               //Sygna³ reset musi trwaæ >=1 ms
	i8080_SIGNALS.OUTSET.reg=i8080_RESET;
	delay_ms(50);				//A przed wys³aniem pierwszego polecenia musimy odczekaæ co najmniej 50 ms
}
