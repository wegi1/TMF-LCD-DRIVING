...
void Text_Demo()
{
	...
	uint16_t ile=200; counter=0; REG_TC6_COUNT32_COUNT=0;
	counter=0;
	while(--ile)
	{
		...
		LCD_SetText(x1, y1+16, "ARM&AVR. LCD-TFT", system16_array, color, 0x000000ul, false);

		if(counterrdy)
		{
			sprintf(bufor, "%5lu b/s, %5lu pix/s", counter, counter/10);
			LCD_SetText(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul, false);
