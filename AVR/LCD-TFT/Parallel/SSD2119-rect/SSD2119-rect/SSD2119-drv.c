...
typedef struct
{
	uint8_t reg;        //Numer rejestru uk³adu SSD
...
void LCD_Rect(uint16_t x1, uint8_t y1, uint16_t x2, uint8_t y2, __uint24 color)
{
	...
	for(;val;val--)
	{
		i8080_Write_W(color >> 8);		//Wyœlij RG
		i8080_Write_B(color);			//Wyœlij B na liniach D0-D7
	}
	LCD_CS(1);
}

void LCD_Rect16b(uint16_t x1, uint8_t y1, uint16_t x2, uint8_t y2, __uint24 color)
{
	LCD_SetWindow(x1, y1, x2, y2);
	LCD_SetPosition(x1, y1);
	LCD_SendCmd(ssd2119_Write_to_GRAM);    // Zapis pod wskazan¹ pozycjê
	uint16_t w1=color >> 8;                             //Komponenty RG koloru
	uint16_t w2=((color & 0xff) << 8) | (color >> 16);  //Komponenty BR
	uint16_t w3=color & 0xffff;                         //Komponenty GB

	__uint24 val=(1+y2-y1)*(__uint24)(1+x2-x1) + 1;  //Policz z ilu pikseli sk³ada siê prostok¹t
	if(val & 1)			//Wyœlij piksel jeœli liczba pikseli w prostok¹cie jest nieparzysta
	{
		i8080_Write_W(color >> 8);
		i8080_Write_B(color);
	}
	
	ssd2119_SendCmdWithData(ssd2119_Entry_Mode, (ssd2119_EntryMode_Reg){.DFM=0b10, .DenMode=1, .TY=0b00, .ID=0b11, .AM=0}.word);  //Tryb 666 z adresowaniem 2 piksele/3 transakcje	
	LCD_SendCmd(ssd2119_Write_to_GRAM);    // Zapis pod wskazan¹ pozycjê
	for(;val/2;val--)
	{
		i8080_Write_W(w1);				//Wyœlij RG pierwszego piksela
		i8080_Write_W(w2);				//Wyœlij B pierwszego i R drugiego piksela
		i8080_Write_W(w3);				//Wyœlij GB drugiego piksela piksela
	}
	ssd2119_SendCmdWithData(ssd2119_Entry_Mode, (ssd2119_EntryMode_Reg){.DFM=0b10, .DenMode=1, .TY=0b01, .ID=0b11, .AM=0}.word);  //Tryb 666 z adresowaniem 1 piksel/2 transakcje
	LCD_CS(1);
}
	...
	for(; height; height--)
	{
		...
		while(owidth)
		{
			...
			while(cnt<dx*3)
			{
				...
				uint16_t pix=i8080_Read_W();   //Odczytaj dwa bajty opisu piksela
				bytetbl[cnt]=(alfa*(bmppixel.red << 3) + alfaneg*(pix>>8)) >> 8;
				bytetbl[cnt+1]=(alfa*(bmppixel.green << 2) + alfaneg*(pix & 0xff)) >> 8;
				pix=i8080_Read_W();				//Na mniej znacz¹cym bajcie jest sk³adowa blue piksela
				bytetbl[cnt+2]=(alfa*(bmppixel.blue << 3) + alfaneg*(pix>>8)) >> 8;
				cnt+=3;
			}
			LCD_SetPosition(ox, y);
			LCD_SendCmd(ssd2119_Write_to_GRAM);    // Zapis pod wskazan¹ pozycjê
			...
			while(cnt<dx*3)
			{
				i8080_Write_W((bytetbl[cnt]<<8) | bytetbl[cnt+1]);
				i8080_Write_B(bytetbl[cnt+2]);
				cnt+=3;
			}
			ox+=dx; owidth-=dx;
		}
...
static void setPixelAA(uint16_t x1, uint8_t y1, uint8_t alfaneg, __uint24 color, __uint24 bkgcolor)
{
	...
	uint8_t alfa=~alfaneg;
	uint16_t pix=((alfa*((LCD_RGB)color).red + alfaneg*(((LCD_RGB)bkgcolor).red)) >> 8);     //Blending kolorów
	pix=(pix<<8) | ((alfa*((LCD_RGB)color).green + alfaneg*(((LCD_RGB)bkgcolor).green)) >> 8);
	i8080_Write_W(pix);
...
void LCD_DrawBitmap_565(uint16_t x, uint8_t y, const uint16_t __flash *data)
{
	...
	i8080_Write_W(*data);   //Przeœlij ostatni piksel
