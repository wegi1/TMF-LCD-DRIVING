...
static __attribute__((always_inline)) inline void i8080_Write_B(uint8_t w8) //Zapisz bajt na pozycji D0-D7, zak³ada, ¿e CS jest aktywny
{
	LCD_WR(0);
	VPORT0_OUT=w8 & 0xff;
	LCD_WR(1);
...
static __attribute__((always_inline)) inline uint16_t i8080_Read_W() //Odczytaj s³owo, zak³ada, ¿e CS jest aktywny
{
	...
	uint16_t data=VPORT0_IN | (VPORT1_IN << 8);
	LCD_RD(1);
...
static inline void LCD_Interface_Init()
{
	...
	VPORT0_DIR=0xff;  //Szyna danych jest wyjœciem
	VPORT1_DIR=0xff; 
