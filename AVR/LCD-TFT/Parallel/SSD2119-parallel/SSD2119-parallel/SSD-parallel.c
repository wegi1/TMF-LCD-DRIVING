...
void Text_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%7lu b/s, %6lu pix/s"), counter, counter/10);
...
void Line_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%7lu b/s, %7lu pix/s"), counter, counter/10);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			Reset_Counter();
		}
	}
...
void Bitmap_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%5lu b/s, %5lu pix/s"), counter, counter/2);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			Reset_Counter();
		}
	}
...
void Circle_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%5lu b/s, %5lu pix/s"), counter, counter/10);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			Reset_Counter();
		}
	}
...
int main(void)
{
	...
	LCD_Rect(0, 0, LCD_GetMaxX()-1, LCD_GetMaxY()-1, 0x000000ul);
