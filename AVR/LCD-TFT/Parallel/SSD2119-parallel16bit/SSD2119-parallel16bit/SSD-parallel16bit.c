...
void Text_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%7lu b/s, %6lu pix/s"), counter*2, counter/50);
...
void Line_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%7lu b/s, %7lu pix/s"), counter*2, counter/5);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			Reset_Counter();
		}
	}
...
void Circle_Demo()
{
	...
	while(--ile)
	{
		...
		if(counterrdy)
		{
			sprintf_P(bufor, PSTR("%5lu b/s, %5lu pix/s"), counter*2, counter/5);
			LCD_SetTextOpaque(0, 0, bufor, system16_array, 0xfffffful, 0x000000ul);
			Reset_Counter();
		}
	}
...
void AAText_Demo()
{
	...
	while(--ile)
	{
		...
		uint8_t  y1=rand() % (LCD_GetMaxY()-16);   //Maksymalna wspó³rzêdna y
		__uint24 color=rand() + (((__uint24)rand()) << 8) + (((__uint24)rand()) << 16); //Wybierz kolor
		LCD_SetTextAA(x1, y1+16, PSTR("ARM&AVR. LCD-TFT"), Times16AA332_array, color, 0x000000ul);
