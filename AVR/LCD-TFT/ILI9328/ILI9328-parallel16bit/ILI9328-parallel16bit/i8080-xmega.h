...
static __attribute__((always_inline)) inline uint8_t i8080_Read_B()   //Odczytaj bajt, zak³ada, ¿e CS jest aktywny
{
	...
	LCD_RD(0);
	_delay_us(i8080_RDL);    //Dane pojawiaj¹ siê najwczeœniej po 100 ns, a niski stan CS musi trwaæ co najmniej 150 ns
...
static __attribute__((always_inline)) inline uint16_t i8080_Read_W() //Odczytaj s³owo, zak³ada, ¿e CS jest aktywny
{
	...
	VPORT1_DIR=0x00;   //Prze³¹czamy port na wejœcie
	LCD_RD(0);
	_delay_us(i8080_RDL);    //Dane pojawiaj¹ siê najwczeœniej po 100 ns, a niski stan CS musi trwaæ co najmniej 500 ns
	uint16_t data=VPORT1_IN << 8;
	data|=VPORT0_IN;
	LCD_RD(1);
	//_delay_us(i8080_RDL);    //Dane pojawiaj¹ siê najwczeœniej po 250 ns, a niski stan CS musi trwaæ co najmniej 150 ns
	VPORT0_DIR=0xff;
	VPORT1_DIR=0xff;  //Z powrotem jest wyjœciem
	return data;
}
...
static inline void ILI_RESET()
{
	...
	_delay_ms(1);               //Sygna³ reset musi trwaæ >=1 ms
	VPORT2_OUT|=i8080_RESET;
	_delay_ms(50);				//A przed wys³aniem pierwszego polecenia musimy odczekaæ co najmniej 50 ms
}
