...
void LCD_Init262()
{
	...
	ILI_SendCmdWithData(ILI9328_ENTRYMODE, 0xD030);             // Entry Mode (R03h) - RGB 2 transfery/pixel, dwa najstarsze bity R + reszta R i GB w kolejnym bajcie
...
//Funkcja wysy³a przekodowane sk³adowe RGB przez interfejs 16-bitowy
static inline void LCD_WriteRGB(uint8_t red, uint8_t green, uint8_t blue)
{
	i8080_Write_B(red);   //Wyœlij dwa najbardziej znacz¹ce bity sk³adowej czerwonej
	i8080_Write_W((blue >> 2) | ((green & 0b11111100) << 4) | ((red & 0b11111100) << 10));  //Wyœlij sk³adowe RGB
}

//Funkcja wysy³a przekodowane sk³adowe RGB przez interfejs 16-bitowy
static inline void LCD_Write24(__uint24 color)
{
	uint8_t red=color >> 16;
	uint8_t green=(color >> 8) & 0xff;
	uint8_t blue=color & 0xff;
	LCD_WriteRGB(red, green, blue);
}
...
void LCD_Rect(uint16_t x1, uint8_t y1, uint16_t x2, uint8_t y2, __uint24 color)
{
	...
	ILI_ColorMapping col=(ILI_ColorMapping){.red=color >> 18, .green=(color >> 10) & 0xff, .blue=(color >> 2) & 0xff};
	
	__uint24 val=(1+y2-y1)*(__uint24)(1+x2-x1) + 1;  //Policz z ilu pikseli sk³ada siê prostok¹t
	for(;val;val--)
	{
		i8080_Write_B(col.HiByte);
		i8080_Write_W(col.LoWord);
	}
	LCD_CS(1);
}
...
void LCD_DrawBitmap_565_Alpha(uint16_t x, uint8_t y, uint8_t alfaneg, const uint16_t __flash *data)
{
	...
	for(; val>0; val--)
	{
		...
		ILI_ColorMapping col=(ILI_ColorMapping){.red=red >> 2, .green=green >> 2, .blue=blue >> 2};
		i8080_Write_B(col.HiByte);
		i8080_Write_W(col.LoWord);
	}
	LCD_CS(1);
...
void LCD_LineTo(uint16_t x, uint8_t y, uint16_t x2, uint8_t y2, __uint24 color)
{
	...
	void LCD_SetPixel(uint16_t x1, uint8_t y1)
	{
		LCD_SetPosition(x1, y1);
		LCD_SendCmd(ILI_Write_to_GRAM);    // Zapis pod wskazan¹ pozycjê
		i8080_Write_B(col.HiByte);
...
static void setPixelAA(uint16_t x1, uint8_t y1, uint8_t alfaneg, __uint24 color, __uint24 bkgcolor)
{
	...
	uint8_t alfa=~alfaneg;
	uint8_t red=(alfa*((LCD_RGB)color).red + alfaneg*(((LCD_RGB)bkgcolor).red)) >> 8;     //Blending kolorów
	uint8_t green=(alfa*((LCD_RGB)color).green + alfaneg*(((LCD_RGB)bkgcolor).green)) >> 8;
	uint8_t blue=(alfa*((LCD_RGB)color).blue + alfaneg*(((LCD_RGB)bkgcolor).blue)) >> 8;
	ILI_ColorMapping col=(ILI_ColorMapping){.red=red >> 2, .green=green >> 2, .blue=blue >> 2};
	i8080_Write_B(col.HiByte);
	i8080_Write_W(col.LoWord);
}
...
void LCD_SetTextOpaque(uint16_t x, uint8_t y, const __memx char *tekst, const uint8_t __flash * const __flash font[], __uint24 color, __uint24 bkgcolor)
{
	...
	//Zmapuj koloroy na format u¿ywany przez ILI	
	ILI_ColorMapping pixcol=(ILI_ColorMapping){.red=color >> 18, .green=(color >> 10) & 0xff, .blue=(color >> 2) & 0xff};
	ILI_ColorMapping bkgcol=(ILI_ColorMapping){.red=bkgcolor >> 18, .green=(bkgcolor >> 10) & 0xff, .blue=(bkgcolor >> 2) & 0xff};
	...
	while((ch=*tekst++))  //Wyœwietl kolejne znaki a¿ do koñca tekstu (znaku NUL)
	{
		...
		for(uint8_t ox=0; ox < col; ox++)        //Wyœwietlamy kolejne kolumny tworz¹ce znak
		{
			for(uint8_t oy=0; oy < rows; oy++)   //Narysuj jedn¹ kolumnê znaku
			{
				...
				if(coldesc & 0x80) 
				{
					i8080_Write_B(pixcol.HiByte);
					i8080_Write_W(pixcol.LoWord);
				} else          //Narysuj t³o tylko jeœli nie wyœwietlamy przeŸroczystego tekstu
				{
					i8080_Write_B(bkgcol.HiByte);
					i8080_Write_W(bkgcol.LoWord);
				}
				coldesc<<=1;
			}
			x++;  //Rysujemy kolejn¹ kolumnê
...
//
	void CharRenderFunc1bitNoAA(uint16_t pixelsno, uint8_t rows, const uint8_t __flash *znak, __uint24 color, __uint24 bkgcolor)
	{
		...
		for(uint16_t ox=0; ox < pixelsno; ox++)        //Wyœwietlamy kolejne kolumny tworz¹ce znak
		{
			...
			ILI_ColorMapping col=(ILI_ColorMapping){.red=pixcolor >> 18, .green=(pixcolor >> 10) & 0xff, .blue=(pixcolor >> 2) & 0xff};
			i8080_Write_B(col.HiByte);
			i8080_Write_W(col.LoWord);
			coldesc<<=1;
		}	
