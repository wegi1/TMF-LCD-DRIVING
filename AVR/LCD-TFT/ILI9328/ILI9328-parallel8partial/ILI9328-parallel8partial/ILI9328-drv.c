...
void LCD_Init262()
{
	...
	_delay_us(500);
	ILI_SendCmdWithData(ILI9328_GAMMAFINEADJREG_PP0, 0x0000);         // Gamma Control 1
	ILI_SendCmdWithData(ILI9328_GAMMAFINEADJREG_PP1, 0x0107);         // Gamma Control 2
	ILI_SendCmdWithData(ILI9328_GAMMAFINEADJREG_PP2, 0x0000);         // Gamma Control 3
	ILI_SendCmdWithData(ILI9328_GAMMAGRADIENTADJREG_PP, 0x0203);         // Gamma Control 4
	ILI_SendCmdWithData(ILI9328_GAMMAAMPLITUDEADJREG_PP	, 0x0402);         // Gamma Control 5
	ILI_SendCmdWithData(ILI9328_GAMMAFINEADJREG_NP0, 0x0000);         // Gamma Control 6
	ILI_SendCmdWithData(ILI9328_GAMMAFINEADJREG_NP1, 0x0207);         // Gamma Control 7
	ILI_SendCmdWithData(ILI9328_GAMMAFINEADJREG_NP2, 0x0000);         // Gamma Control 8
	ILI_SendCmdWithData(ILI9328_GAMMAGRADIENTADJREG_NP	, 0x0203);         // Gamma Control 9
	ILI_SendCmdWithData(ILI9328_GAMMAAMPLITUDEADJREG_NP	, 0x0403);        // Gamma Control 10
...
//
	...
	switch((uint8_t)(uint16_t)font[0])
	{
		case 0x01:       RenderFuncPtr=&CharRenderFunc1bitNoAA;  //Renderujemy czcionkê bez antyaliasingiem 1bpp
						 ILI_SendCmdWithData(ILI9328_ENTRYMODE, (ILI9328_EntryMode_Reg){.TRI=1, .DFM=1, .ID=0b01, .AM=1}.word);
						 break;
	...
	LCD_CS(1);
	ILI_SendCmdWithData(ILI9328_ENTRYMODE, (ILI9328_EntryMode_Reg){.TRI=1, .DFM=1, .ID=0b11, .AM=0}.word);
}
...
void LCD_DrawBitmap_Scale(uint16_t x, uint8_t y, uint8_t scale, const uint16_t __flash *data)
{
	ILI_RGB565();     //Prze³¹cz siê na tryb przesy³ania 2 bajty na piksel

	uint16_t width=*data++;
	uint8_t height=*data++;

	LCD_SetWindow(x, y, x + (width / scale) -1, y +  (height / scale) - 1);
	LCD_SetPosition(x, y);
	LCD_SendCmd(ILI_Write_to_GRAM);    // Zapis pod wskazan¹ pozycjê
	...
	LCD_CS(1);
	ILI_RGB666();  //Wróæ do trybu 3 bajty/piksel
}