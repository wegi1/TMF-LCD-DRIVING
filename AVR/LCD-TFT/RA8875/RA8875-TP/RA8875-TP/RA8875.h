...
typedef union
{
	struct
	{
		...
		uint8_t CursorType     : 3;			//Typ kursora - 0-7
...
#define RA_Extra_General_Purpose_IO_Register							0xC5
...
uint8_t RA_SendCmdReadData(uint8_t cmd)  __attribute__ ((flatten)); //Wyœlij poecenie i odczytaj parametr z rejestru kontrolera
...
void RA_SetScrollWindow(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2); //Ustaw okienko przewijania
void RA_SetScrollOffset(uint16_t horizontal, uint16_t vertical); //Przewiñ okno o wybran¹ liczbê pikseli
...
static inline void RA_WaitForWAIT() {asm volatile ("nop"); asm volatile ("nop"); while(!(VPORT2_IN & RA8875_WAIT));}  //Zaczekaj na deaktywacjê sygna³u WAIT z kontrolera
...
//Kursor graficzny
void RA_SetGraphCursorPos(uint16_t x, uint16_t y);		            //Ustaw pozycjê kursora graficznego
void RA_SetGraphCursorColor(uint8_t col1, uint8_t col2);            //Ustaw kolor kursora
void RA_SetCurShape(uint8_t curType, const __flash uint8_t *data);  //Zdefiniuj ksta³t kursora o podanym numerze
void RA_GraphCursorEn(_Bool en, uint8_t curType);                   //Wyœwietl kursor o podanym numerze
...
void LCD_SetText(uint16_t x, uint16_t y, const __memx char *tekst, RA_FNType FontType, _Bool CGRAM);  //Wyœwietl podany ³añcuch sprzêtowo
...
uint16_t LCD_StoreBMPMonoInGRAM(uint16_t x, uint16_t y, const uint8_t __flash *data, uint8_t Layer);  //Zapisuje monochromatyczn¹ bitmapê w GRAM na wybranej warstwie - dzia³a w trybie 8bpp
...
void RA_DrawBitmap332(uint16_t x, uint16_t y, const uint8_t __flash *data, uint8_t layer); //Wyœwietl obraz w formacie 332 na ekranie w trybie 8bpp
