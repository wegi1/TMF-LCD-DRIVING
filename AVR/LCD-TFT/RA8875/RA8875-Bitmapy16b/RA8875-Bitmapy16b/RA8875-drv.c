...
void LCD_Init256()
{
	LCD_Init65k();  //Najpierw zainicjuj kontroler w trybie 18-bitowego koloru
	...
	RA_SendCmdWithData(RA_Display_Configuration_Register, (RS8875_DPCR_Reg){.VDIR=0, .HDIR=0, .Layers=1}.byte);	//W³¹cz dwie warstwy - bez tego nie dzia³a wybór warstw we wszystkich funkcjach
}
...
void LCD_DrawBitmap_Mono(uint16_t x, uint16_t y, uint16_t color, uint16_t bkgcolor, const uint8_t __flash *data, _Bool transparency)
{
