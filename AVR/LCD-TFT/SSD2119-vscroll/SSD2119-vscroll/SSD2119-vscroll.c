...
void ssd2119_VScroll()
{
	uint8_t vscroll=0;

	while(1)
	{
		ssd2119_SendCmdWithData(ssd2119_GateScan_Position, vscroll + 1);
		vscroll=(vscroll+1) % 240;
		_delay_ms(20);
	}
}

void ssd2119_VScroll_Ctrl()
{
	uint8_t vscroll=0;
	ssd2119_SendCmdWithData(ssd2119_Display_Control, (ssd2119_DisplayCtrl_Reg){.D0=1, .D1=1, .DTE=1, .GON=1, .CM=1, .VLE=1}.word);
	while(1)
	{
		ssd2119_SendCmdWithData(ssd2119_Vertical_Scroll_Control_Screen1, vscroll + 1);
		vscroll=(vscroll+1) % 240;
		_delay_ms(20);
	}
}
...
int main(void)
{
	...
	SSD2119_Rect(70, 0, 180, 0, 0x00FF00UL); //Narysuj niebieski prostok¹t w lewym górnym rogu

	ssd2119_SendCmdWithData(ssd2119_Driver_Output_Control, (ssd2119_DriverOutputCtrl_Reg){.RL=0, .REV=1, .GD=1, .TB=1, .MUX=239}.word);
	SSD2119_Rect(10, 10, 50, 50, 0xFF0000UL); //Narysuj niebieski prostok¹t w prawym górnym rogu
	SSD2119_Rect(10, 100, 310, 150, 0xFF00FFUL);

	//ssd2119_VScroll();     //P³ynne przewijanie ca³ego obrazu
	ssd2119_VScroll_Ctrl();  //P³ynne przewijanie kranu z wykorzystaniem Vertical Scroll Control Register
