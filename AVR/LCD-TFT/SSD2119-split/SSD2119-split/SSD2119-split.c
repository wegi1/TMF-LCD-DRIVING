...
void ssd2119_DrawBitmapLine_Mono(uint16_t x, uint8_t y, uint16_t width,  uint8_t line, const uint8_t __flash *data)
{
...
void ssd2119_DrawBitmap_Mono(uint16_t x, uint8_t y, uint16_t width, uint8_t height, const uint8_t __flash *data)
{
...
void ssd2119_DrawBitmapLine_565(uint16_t x, uint8_t y, uint16_t width, uint8_t line, const uint16_t __flash *data)
{
	SSD2119_SetWindow(x, y, x + width -1, y);
	SSD2119_SetPosition(x, y);
	ssd2119_SendCmd(0x22);    // Zapis pod wskazan¹ pozycjê
...
void ssd2119_DrawBitmap_565(uint16_t x, uint8_t y, uint16_t width, uint8_t height, const uint16_t __flash *data)
{
	SSD2119_SetWindow(x, y, x + width -1, y +  height - 1);
	SSD2119_SetPosition(x, y);
	ssd2119_SendCmd(0x22);    // Zapis pod wskazan¹ pozycjê
...
int main(void)
{
	...
	ssd2119_SendCmdWithData(ssd2119_Display_Control,(ssd2119_DisplayCtrl_Reg){.D0=1, .D1=1, .DTE=1, .GON=1, .VLE=0b11, .SPT=1}.word); //Podziel ekran

	SED2119_RGB565();  //W³¹cz tryb RGB 5-6-5

	ssd2119_DrawBitmap_565(0, 0, 320, 48, image_data_colicons1);
	ssd2119_DrawBitmap_565(0, 48, 320, 48, image_data_colicons2);

	ssd2119_SendCmdWithData(ssd2119_First_Screen_Driving_Position_Start, 0);
	ssd2119_SendCmdWithData(ssd2119_First_Screen_Driving_Position_End, 47);

	uint8_t y=48, dir=1;
	while(1)
	{
		ssd2119_SendCmdWithData(ssd2119_Second_Screen_Driving_Position_Start, y); //Drugi ekran - pocz¹tek
		ssd2119_SendCmdWithData(ssd2119_Second_Screen_Driving_Position_End, y+48);
		ssd2119_SendCmdWithData(ssd2119_Vertical_Scroll_Control_Screen2, 31-y);
		_delay_ms(30);
		y+=dir;
		if(y>=192) dir=-1;  //Zmieñ kierunek przesuwu na w górê
		if(y<=48) dir=1;    //Zmieñ kierunek przesuwu na w dó³
	}
}