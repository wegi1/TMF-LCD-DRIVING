...
{
	ssd2119_CS(0);
	SPI_R_Byte(data >> 8);  //Wyœlij bardziej znacz¹cy bajt danych
	ssd2119_CS(1);
	ssd2119_CS(0);
	SPI_R_Byte(data & 0xff);  //Wyœlij mniej znacz¹cy bajt danych
	ssd2119_CS(1);
}
...
static inline void ssd2119_SendCmd(uint8_t cmd)
{
	...
	ssd2119_CS(0);
	SPI_R_Byte(cmd);
	ssd2119_RS(1);     //Deaktywuj tryb wysy³ania poleceñ, CS zostawiamy aktywny, bo trzeba wys³aæ dane dla polecenia
}
...
static inline void LCD_Interface_Init()
{
	...
	VPORT0_DIR=LCD_CS | LCD_RS | TP_CS | LCD_RESET | LCD_SCK | LCD_MOSI; //Ustaw odpowiednie piny jako wyjœcia
	...
	LCD_USART.CTRLB=USART_TXEN_bm | USART_RXEN_bm;  //W³¹czamy tak¿e odbiornik ze wzglêdu na kontroler panela
}
