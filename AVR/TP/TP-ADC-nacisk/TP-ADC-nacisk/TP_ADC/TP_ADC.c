...
static const __flash TP_IO_Cfg Touch_IO_Cfg[]={{.ADCMUX=ADC_CH_MUXPOS_PIN4_gc | ADC_CH_MUXNEG_PIN7_gc, .PORTSET=PIN5_bm, .PORTCLR=PIN7_bm},  //Pomiar wsp. X
	                                           {.ADCMUX=ADC_CH_MUXPOS_PIN5_gc | ADC_CH_MUXNEG_PIN6_gc, .PORTSET=PIN4_bm, .PORTCLR=PIN6_bm},  //Pomiar wsp. Y
											   {.ADCMUX=ADC_CH_MUXPOS_PIN4_gc | ADC_CH_MUXNEG_PIN6_gc, .PORTSET=PIN5_bm, .PORTCLR=PIN6_bm},  //Pomiar si³y nacisku - Z1
											   {.ADCMUX=ADC_CH_MUXPOS_PIN7_gc | ADC_CH_MUXNEG_PIN6_gc, .PORTSET=PIN5_bm, .PORTCLR=PIN6_bm},  //Pomiar si³y nacisku - Z2
											   {.PORTCLR=PIN7_bm}};   //Stan spoczynkowy
...
static uint16_t TP_ADCResult(TouchPanel_Cord ctrl_byte)
{
	...
	PORTA_OUTCLR=Touch_IO_Cfg[4].PORTCLR;  //Wymuœ GND na Y-, dziêki czemu bêdziemy mogli wykrywaæ dotyk poprzez zwarcie X+ do masy
	PORTA_DIRCLR=0b11110000;
	PORTA_DIRSET=Touch_IO_Cfg[4].PORTCLR;
	return result;
}
...
uint16_t TouchPanel_GetPositionAndTouchXY(TP_Position *pos)
{
	TouchPanel_GetPositionXY(pos);            //Pobierz wspó³rzêdne XY
	int16_t Z1=TP_ADCResult(TouchPanel_Z1);   //Pobierz napiêcie w punkcie Z1
	int16_t Z2=TP_ADCResult(TouchPanel_Z2);   //Pobierz napiêcie w punkcie Z2
	return ((pos->Y*(int32_t)Z2)/Z1 - pos->Y);  //Powinno byæ X, ale mamy odwrócony panel
}